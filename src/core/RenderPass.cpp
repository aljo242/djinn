...
Djinn::RenderPass::RenderPass(Djinn::Context* p_context, const RenderPassConfig& config)
{
	Init(p_context, config);
}

void Djinn::RenderPass::Init(Djinn::Context* p_context, const RenderPassConfig& config)
{
	// create attachment
	...
	colorAttachmentResolve.format = config.swapChainFormat;
	...
	auto result{ (vkCreateRenderPass(p_context->gpuInfo.device, &renderPassInfo, nullptr, &handle)) };
	DJINN_VK_ASSERT(result);
}

void Djinn::RenderPass::CleanUp(Djinn::Context* p_context)
{
	vkDestroyRenderPass(p_context->gpuInfo.device, handle, nullptr);
}
